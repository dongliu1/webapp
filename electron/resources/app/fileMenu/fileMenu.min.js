...
(function(n) {
	(function(n) {
		(function(n) {
			...
			var i = function() {
					function i() {
						this.recentFiles = [], this.currentFolder = null, this.recentFolders = [], this.mainMenu = new t($(".menu-panel ul")), this.openMenu = new t($("#open-page .left-view ul")), this.importMenu = new t($("#import-page .left-view ul")), this.exportMenu = new t($("#export-page .left-view ul")), app.config("recentFiles") !== undefined && (this.recentFiles = app.config("recentFiles")), app.config("currentFolder") !== undefined && (this.currentFolder = app.config("currentFolder")), app.config("recentFolders") !== undefined && (this.recentFolders = app.config("recentFolders"))
					}
					return i.prototype._updateRecentFileList = function(n) {
						...
						for (i = 0; i < this.recentFiles.length; i++) if (r === this.recentFiles[i].path + "\\" + this.recentFiles[i].name) {
							u = this.recentFiles[i], this.recentFiles.splice(i, 1), e = !0;
							break
						}
						e || (u = {
							iconClass: t,
							path: o,
							name: h
						}), f = this.recentFiles.unshift(u), f > 20 && this.recentFiles.splice(20, f - 20), app.config("recentFiles", this.recentFiles)
					}, i.prototype._updateRecentFolderList = function(n) {
						for (var e = n.replace(/\//g, "\\"), i = e.substring(0, e.lastIndexOf("\\")), f = i.substring(i.lastIndexOf("\\") + 1), o = !1, r, u, t = 0; t < this.recentFolders.length; t++) if (i === this.recentFolders[t].path) {
							r = this.recentFolders[t], this.recentFolders.splice(t, 1), o = !0;
							break
						}
						o || (r = {
							path: i,
							...
							path: i,
							name: f
						}, u = this.recentFolders.unshift(r), u > 20 && this.recentFolders.splice(20, u - 20), app.config("currentFolder", this.currentFolder), app.config("recentFolders", this.recentFolders)
					}, i.prototype.updateRecentList = function(n) {
						this._updateRecentFileList(n), this._updateRecentFolderList(n), this._fillRecentWorkbooksPage(this.recentFiles), this._fillCurrentFolderList(this.currentFolder), this._fillRecentFoldersList(this.recentFolders)
						...
						if (!t) return;
						for (t.children().length > 0 && t.empty(), $("<option>").text(n.res.fileMenu.allSheet).appendTo(t), i = 0; i < r; i++) $("<option>").text(u[i].getName()).appendTo(t)
						...
						if (n.target.getAttribute("href") === "#") return;
						...
						var t = this;
						this._initAllPages(), $(".menu-title span").text(n.res.fileMenu._new), this.mainMenu.showTab("#new-page"), this.bindNavigatorHandlers();
						...
						$("#filemenu-options").on("click", function() {
							return t._showOptionsDialog()
						});
					...
					}, i.prototype._showOptionsDialog = function() {
						this.optionsDialog || (this.optionsDialog = new n.OptionsDialog), this.optionsDialog.open()
					}, i.prototype._initAllPages = function() {
						this._initNewPage(), this._initOpenPage(), this._initSaveAsPage(), this._initImportPage(), this._initExportPage()
						...
						var u = this,
							t = $("<div>"),
							r, i;
						t.addClass("template-item"), r = $("<div>"), r.addClass("template-item-thumb").addClass(n.thumbClass), r.appendTo(t), i = $("<span>"), i.text(n.name), i.appendTo(t);
						t.on("click", function() {
						...
						for (var r = $("#new-page"), i, t = 0; t < n.length; t++) i = this._createTemplateItem(n[t]), i.appendTo(r)
					}, i.prototype._initOpenPage = function() {
						this._fillRecentWorkbooksPage(this.recentFiles), this._fillComputerPage(this.currentFolder, this.recentFolders)
						...
						var e = this,
							...
							f = $("<div>"),
							u = $("<div>");
						i.addClass("file-label"), t.addClass("list-item"), r.addClass("list-item-big-icon").addClass(n.iconClass), f.text(n.name).addClass("file-name").appendTo(i), u.text(n.path).addClass("file-path").appendTo(i), t.append(r).append(i), t.addClass("only-for-licensed");
						t.on("click", function() {
						...
						var u, r, i, e, f;
						$("#recent-workbooks-page").find("span").remove(), $("#recent-workbooks-page").find("ul").remove();
						if (t.length === 0) u = $("<span>"), u.text(n.res.fileMenu.noRecentWorkbooks), u.appendTo($("#recent-workbooks-page"));
						else for (r = $("<ul>"), r.appendTo($("#recent-workbooks-page")), i = 0; i < t.length; i++) e = t[i], f = this._createFileItem(e), f.appendTo(r)
						...
						var e = this,
							...
							f = $("<div>"),
							u = $("<div>");
						i.addClass("file-label"), t.addClass("list-item"), r.addClass("directory-icon"), f.text(n.name).addClass("file-name").appendTo(i), u.text(n.path).addClass("file-path").appendTo(i), t.append(r).append(i), t.addClass("only-for-licensed");
						...
						if (n === null) $(".current-folder").prev().hide(), $(".current-folder").hide();
						else {
							$(".current-folder").prev().show(), $(".current-folder").show();
							var t = $("<ul>"),
								i = this._createFolderItem(n);
							i.appendTo(t), $(".current-folder").append(t)
						}
					}, i.prototype._fillRecentFoldersList = function(t) {
						var u, r, i, f;
						$(".recent-folders").find("span").remove(), $(".recent-folders").find("ul").remove();
						if (t.length === 0) u = $("<span>"), u.text(n.res.fileMenu.noRecentFolders), u.appendTo($(".recent-folders"));
						else {
							for (r = $("<ul>"), i = 0; i < t.length; i++) f = this._createFolderItem(t[i]), f.appendTo(r);
							$(".recent-folders").append(r)
						}
					}, i.prototype._fillComputerPage = function(n, t) {
						this._fillCurrentFolderList(n), this._fillRecentFoldersList(t), this._addOpenBrowserListener()
						...
						$(".export-pdf-printer").on("click", function(i) {
							t._exportPDFPrinter === undefined && (t._exportPDFPrinter = new n.PDFPrinterDialog), t._exportPDFPrinter.open()
						})
					}, i.prototype.restrictFeatures = function() {
						$(".only-for-licensed").prop("disabled", "disabled"), $(".only-for-licensed").css("-webkit-filter", "grayscale(100%)"), $(".only-for-licensed").off("click"), $(window).off("keypress.licensed-feature")
					}, i._intParser = function(n) {
						return n = parseInt(n), isNaN(n) ? null : n
					}, i._delimiterParser = function(n) {
						return n.replace(/\\r/g, "\r").replace(/\\n/g, "\n")
					}, i
				}(),
				t;
			n.FileMenu = i, t = function() {
				function n(n) {
					this.rootElem = n, this.selectedTab = null, this.onSelectItem = null, this._bindClickHandler(), this.selectedTab = n.find("a:first").closest("li"), this.showTab(this.selectedTab.children("a").attr("href"))
				}
				return n.prototype.showTab = function(n) {
					...
					this.selectedTab = $("li > a[href=" + n + "]").parent(), this.selectedTab.addClass("menu-item-selected"), this.hideUnselectedViews(), $(n).show("fast")
					...
					this.rootElem.find("a").on("click", function(t) {
						return n._onClickItem(t), !1
					})
				}, n.prototype._onClickItem = function(n) {
					var t = $(n.target).attr("href");
					$(t).length !== 0 && this.showTab(t), this.onSelectItem && this.onSelectItem.call(null, n)
				}, n
			}(), n.Menu = t, n.fileMenu, n.loader.ready(function() {
				n.fileMenu = new i;
				$("#file-menu-tab").on("click", function() {
					n.fileMenu.showFileScreen(), n.fileMenu._initPDFExportSheetSelect()
				});
				n.fileMenu.initPage(), app.isNative && app.getLicenseStatus() !== LicenseStatus.Valid && app.getLicenseStatus() !== LicenseStatus.Trial && n.fileMenu.restrictFeatures()
			})
